...
export default async function MyProgressPage() {
  if (!isDatabaseConfigured) {
    ...
    return <ProgressClient checkIns={demoUser.checkIns as any} targetWeight={null} waterMl={0} />;
  }
  ...
  try {
    const [checkIns, profile, waterLogs] = await Promise.all([
    ...
    return <ProgressClient checkIns={checkIns} targetWeight={profile?.targetWeight ?? null} waterMl={waterMl} />;
  } catch {
    const demoUser = DEMO_TRAINEES.find(t => t.id === userId) ?? DEMO_TRAINEES[0];
    return <ProgressClient checkIns={demoUser.checkIns as any} targetWeight={null} waterMl={0} />;
  }
