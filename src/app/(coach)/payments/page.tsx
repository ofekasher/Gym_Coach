...
const STATUS_STYLE: Record<string, any> = {
  ...
  PENDING: { background: "rgba(245,197,24,0.12)", color: "#F5C518", border: "1px solid rgba(245,197,24,0.2)" },
};
    ...
    <div className="max-w-3xl mx-auto pb-16" dir="rtl">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
        <div style={{ ...CARD, padding: 18 }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, color: "#F5C518" }}>₪{totalRevenue.toLocaleString()}</div>
        </div>
        <div style={{ ...CARD, padding: 18 }}>
      ...
      {/* List */}
      {loading ? (
        <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin" style={{ color: "#F5C518" }} /></div>
      ) : (
              ...
              <div key={t.id} style={CARD}>
                <div style={{ padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }} onClick={() => setExpanded(isOpen ? null : t.id)}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 38, height: 38, borderRadius: "50%", background: "rgba(245,197,24,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#F5C518", fontSize: 14 }}>
                      {t.name?.[0] ?? "?"}
                    </div>
                    <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    {sub ? (
                      ...
                      <button onClick={e => { e.stopPropagation(); setForm({ plan: "MONTHLY", amount: "", notes: "", method: "cash" }); setModal({ type: "sub", traineeId: t.id }); }}
                        style={{ background: "rgba(245,197,24,0.12)", border: "1px solid rgba(245,197,24,0.2)", color: "#F5C518", borderRadius: 999, padding: "5px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                  ...
                  <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "16px 20px" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => { setForm({ plan: sub.plan, amount: String(sub.amount), notes: "", method: "cash" }); setModal({ type: "pay", traineeId: t.id, subId: sub.id }); }}
                        style={{ background: "#F5C518", color: "#111", border: "none", borderRadius: 999, padding: "8px 16px", fontSize: 12, fontWeight: 800, cursor: "pointer", flex: 1 }}>
                        + רשום תשלום
                      </button>
          ...
          <div style={{ ...CARD, padding: 24, width: "100%", maxWidth: 400 }} onClick={e => e.stopPropagation()} dir="rtl">
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={modal.type === "sub" ? saveSubscription : recordPayment} disabled={saving || !form.amount}
                style={{ flex: 1, background: "#F5C518", color: "#111", border: "none", borderRadius: 999, height: 44, fontWeight: 800, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>
