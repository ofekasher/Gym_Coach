      ...
      {editing ? (
        <div>
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <input style={{ ...S.input, borderColor: "rgba(182,255,74,0.3)" }} value={form.coachNote}
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[1,2,3].map(s => (
                <button key={s} type="button" onClick={() => setForm(f => ({ ...f, priority: f.priority === s ? 0 : s }))}
                  ...
                  <span style={{ color: form.priority >= s ? "#b6ff4a" : "#3F3F46" }}>★</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                {(se.priority ?? 0) > 0 && (
                  <span style={{ color: "#b6ff4a", fontSize: 13, letterSpacing: -1 }}>
                    {"★".repeat(se.priority)}
                  </span>
                )}
              </div>
            ...
            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
              ...
              {se.weight && <span style={{ background: "rgba(182,255,74,0.1)", color: "#b6ff4a", borderRadius: 7, padding: "4px 8px", fontSize: 11 }}>{se.weight}ק״ג</span>}
            </div>
          ...
          {se.coachNote && (
            <div style={{ background: "rgba(182,255,74,0.07)", border: "1px solid rgba(182,255,74,0.2)", borderRadius: 8, padding: "5px 10px", fontSize: 12, color: "#b6ff4a", display: "flex", alignItems: "center", gap: 5 }}>
...
<style>
  ...
  .coach-note { background: rgba(182,255,74,0.08); border-right: 3px solid #b6ff4a; padding: 6px 10px; margin-top: 6px; font-size: 12px; border-radius: 4px; }
  .stars { color: #b6ff4a; }
    ...
    return (
      <div style={{ textAlign: "center", padding: "60px 0" }}>
        <div style={{ width: 64, height: 64, borderRadius: "50%", background: "rgba(182,255,74,0.08)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
          <Dumbbell style={{ width: 28, height: 28, color: "#b6ff4a" }} />
        </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }} dir="rtl">
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, padding: "16px 18px", background: "#161618", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 18 }}>
        <div>
          ...
          <span style={{ background: "rgba(182,255,74,0.1)", color: "#b6ff4a", fontSize: 11, fontWeight: 700, borderRadius: 999, padding: "2px 10px" }}>{TEMPLATE_LABELS[plan.template] ?? plan.template}</span>
        </div>
        ...
        return (
          <div key={session.id} style={S.card}>
            <button onClick={() => setOpenSession(isOpen ? null : session.id)} style={{
              ...
              <div style={{ width: 34, height: 34, borderRadius: 10, background: "#b6ff4a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 900, color: "#111", flexShrink: 0 }}>
                {idx + 1}
              </div>
